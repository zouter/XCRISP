...
from indel import indel_out_of_guide_seed_pam
from data import get_mapped_reads_folder
...
def reformat_indel_profile( file_prefix ):
    ...
    if not os.path.isfile(file_prefix + '_mappedindels.txt'):
        ...
        for toks in rdr:
            ...
            oligo_id = toks[1]
            read_id = toks[0].split()[0]
            if oligo_id == "": continue
            ...
            nulls = [(indel, "-", "-", muts) for (null,indel, muts) in zip(toks[1].split(','), toks[2].split(','), toks[3].split(','))]
            ...
            if len(nulls) == 1:
                ...
                ok_nulls = [x for x in nulls if indel_out_of_guide_seed_pam(x[1])]  
                if len(ok_nulls) == 0:
                    ok_nulls = nulls
...
if __name__ == "__main__":
    reformat_indel_profile(sys.argv[1])
