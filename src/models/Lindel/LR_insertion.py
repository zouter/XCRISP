...
def run(data, workdir, prefix="", reg=None, pretrained_model=None, valid_data=None):
    ...
    print("Saving model to ", workdir + '{}x_{}insertion.h5'.format(MIN_READS_PER_TARGET, prefix))
...
def transfer_model(genotype, num_samples, pretrained=True):
    ...
    if pretrained:
        run(train_data, workdir, reg=reg, prefix="transfer_" + str(TRANSFER_LEARNING_RATE) + "_" + mappings[genotype] + "_" + str(num_samples) + "_", pretrained_model=pretrained_model, valid_data=None)
    else:
