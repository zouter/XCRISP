...
def plot_training_curves(history, prefix):
    output_dir = os.environ["LOGS_DIR"] + "LR_indel_transfer/"
...
def train_model(x_train, y_train, reg, x_valid=None, y_valid=None, pretrained_model=None):
    ...
    if pretrained_model is not None:
        ...
        history = model.fit(x_train, y_train, epochs=200, verbose=1, validation_data=(x_valid, y_valid))
        return model, None, history
    elif x_valid is not None and y_valid is not None:
        history = model.fit(x_train, y_train, epochs=200, validation_data=(x_valid, y_valid), verbose=1)
...
def get_X_and_Y(data):
    Y = np.array([(sum(y[1][:-21]), sum(y[1][-21:])) for y in data])
    Y = np.nan_to_num(Y)
...
def run(data, workdir, prefix="", reg=None, pretrained_model=None, valid_data=None):
    ...
    if reg is None:
        ...
        np.save(workdir+'{}mse_l1_indel.npy'.format(prefix),errors_l1)
        np.save(workdir+'{}mse_l2_indel.npy'.format(prefix),errors_l2)
    ...
    m.save(workdir + '{}x_{}indel.h5'.format(MIN_READS_PER_TARGET, prefix))
...
def transfer_model(genotype, num_samples, pretrained=True):
    mappings = {
        ...
        "052218-U2OS-+-LibA-postCas9-rep1_transfertrain": "U2SO",
        "052218-U2OS-+-LibA-postCas9-rep1_transfertest": "U2SO",
        "HAP1_train": "HAP1",
        "HAP1_test": "HAP1",
        "TREX_A_train": "TREX_A",
        "TREX_A_test": "TREX_A",
    ...
    pretrained_model = load_model('./models/{}x_{}indel.h5'.format(MIN_READS_PER_TARGET, ""))
...
if __name__ == "__main__":
    ...
    if sys.argv[2] not in ["baseline", "pretrained"]:
        print("experiment is not valid: ", sys.argv)
        exit()    

    # for num_samples in [2, 5, 10, 20, 50, 100, 200, 500]:
    for num_samples in [2000]:
        transfer_model(sys.argv[1], num_samples, pretrained=sys.argv[2] == "pretrained")
